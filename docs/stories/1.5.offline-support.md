...
- [ ] **Task 1: Create SQLite Schema** (AC: 1)
  - [ ] Create `mobile/src/services/sqlite/migrations.ts`
  - [ ] Define CREATE TABLE for `users` (id, displayName, profilePictureURL, etc.)
  - [ ] Define CREATE TABLE for `conversations` (id, type, name, participants JSON, lastMessage, etc.)
  - [ ] Define CREATE TABLE for `messages` (id, conversationId, senderId, content, timestamp, status, etc.)
  - [ ] Define CREATE TABLE for `pending_messages` (localId, conversationId, content, retryCount, etc.)
  - [ ] Run migrations in `initializeDatabase()` with version checking

- [ ] **Task 2: Create Repository Pattern for SQLite** (AC: 2, 3)
  - [ ] Create `mobile/src/services/sqlite/messageRepository.ts`
  - [ ] Implement `insert(message): Promise<void>`
  - [ ] Implement `bulkInsert(messages): Promise<void>`
  - [ ] Implement `getByConversation(conversationId): Promise<Message[]>`
  - [ ] Implement `update(messageId, updates): Promise<void>`
  - [ ] Create `conversationRepository.ts` with similar CRUD operations
  - [ ] Create `pendingMessageRepository.ts` for pending queue

- [ ] **Task 3: Integrate SQLite Caching in Messages Store** (AC: 2, 3)
  - [ ] In `loadMessages()`: read from SQLite first
  - [ ] Update state immediately with cached messages (instant UI)
  - [ ] Then set up Firestore listener
  - [ ] On Firestore updates: upsert to SQLite + update state
  - [ ] Ensure no duplicate messages (use message.id as primary key)

- [ ] **Task 4: Create Network Monitor** (AC: 6, 9)
  - [ ] Install `@react-native-community/netinfo`
  - [ ] Create `mobile/src/services/network/networkMonitor.ts`
  - [ ] Export `useNetworkStatus()` hook
  - [ ] Listen to NetInfo changes
  - [ ] Return `{ isConnected: boolean, isInternetReachable: boolean }`

- [ ] **Task 5: Create Sync Manager for Pending Messages** (AC: 4, 7, 8)
  - [ ] Create `mobile/src/services/sync/syncManager.ts`
  - [ ] Implement `addToPendingQueue(message): Promise<void>`
  - [ ] Implement `processPendingQueue(): Promise<void>`
  - [ ] On network online: iterate pending_messages, send to Firestore
  - [ ] On success: delete from pending_messages table
  - [ ] On failure: increment retryCount, exponential backoff
  - [ ] Call `processPendingQueue()` on app launch and network reconnect

- [ ] **Task 6: Handle Offline Message Sending** (AC: 4, 5)
  - [ ] In `messagesStore.sendMessage()`: check network status
  - [ ] If offline: insert to pending_messages table
  - [ ] Display message with status: 'sending' (show offline indicator)
  - [ ] When online: sync manager will retry
  - [ ] Update message status to 'sent' after successful sync

- [ ] **Task 7: Add Offline Indicator to UI** (AC: 9)
  - [ ] Create `mobile/src/components/atoms/OfflineIndicator.tsx`
  - [ ] Use `useNetworkStatus()` hook
  - [ ] Show banner at top: "No connection. Messages will send when online."
  - [ ] Style: yellow background, info icon
  - [ ] Add to chat screen and conversations list

- [ ] **Task 8: Test Crash Recovery** (AC: 8, 10)
  - [ ] Add pending message to queue
  - [ ] Force quit app (don't send)
  - [ ] Relaunch app
  - [ ] Verify `processPendingQueue()` is called on launch
  - [ ] Verify message eventually sends to Firestore
